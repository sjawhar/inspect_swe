...
def test_codex_binds_same_model_v2_siblings_from_native_agent_message_recipients() -> (
    None
):
    consumer = CodexConsumer()
    ...
    with span("human_cli", type="agent", id="human-cli"):
        consumer.on_pending(parent)
        consumer.on_complete(parent)
    ...
    assert _span_events(SpanBeginEvent) == ["agent-spawn-1", "agent-spawn-2"]
    assert _span_events(SpanEndEvent) == ["agent-spawn-1"]
    consumer.reset()
    assert _span_events(SpanEndEvent) == ["agent-spawn-1", "agent-spawn-2"]
