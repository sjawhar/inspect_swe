...
def test_run_centaur_keeps_ready_session_on_live_bridge_state(
    ...
    async def on_ready(ready: CentaurSession) -> AsyncIterator[None]:
        ...
        assert ready.invocation == ("/opt/agent-cli/claude", "--model", "claude-sonnet-4-5")
    ...
    async def fake_run(agent: object, state: AgentState) -> AgentState:
        ...
        ready_hook = cast(
            Callable[[], AbstractAsyncContextManager[None]], captured["on_ready"]
        )
        async with ready_hook():
...
def test_refreshing_command_preserves_custom_service_and_cli_serialization(
    ...
    assert (
        asyncio.run(handler("answer", "rebuttal", confidence=0.8)) == "scored"
    )
