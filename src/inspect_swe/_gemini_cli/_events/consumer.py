...
class GeminiConsumer(ModelEventSink):
    ...
    def on_complete(self, event: ModelEvent) -> None:
        ...
        if key is None:
            raise RuntimeError("Gemini bridge ModelEvent completed before it was pending")
        pending = self._pending_model_events.get(key)
        if pending is None or pending.event is not event:
            raise RuntimeError("Gemini bridge ModelEvent completion lost its pending identity")
    ...
    async def _drain_configured_telemetry(self) -> None:
        if self._sandbox is None or self._telemetry_path is None:
            raise RuntimeError("Gemini telemetry refresh requires sandbox and telemetry path")
        await self.process_telemetry_from_sandbox(self._sandbox, self._telemetry_path)

    async def process_telemetry_from_sandbox(
        self, sandbox: SandboxEnvironment, telemetry_path: str
    ) -> None:
    ...
    def process_telemetry(self, contents: str) -> None:
        ...
        ready_compactions = [
            ...
            or _span_key_from_parts(compaction.trace_id, compaction.span_id) in ready_keys
        ]
        ...
        timeline: list[
            tuple[_HrTime, int, int, _NativeSpan | _NativeCompaction]
        ] = []
    ...
    def _stage_spans(self, spans: list[_NativeSpan]) -> None:
        for span in spans:
            ...
            if existing is None:
                ...
                raise ValueError(f"conflicting Gemini telemetry records for span {key!r}")
...
def _decode_concatenated_json(contents: str) -> list[_JsonObject]:
    ...
    while True:
        ...
        try:
            ...
            raise ValueError(f"invalid JSON in Gemini telemetry at byte {ex.pos}") from ex
